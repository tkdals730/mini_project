...
| 순서 | 단계 | 현재 상태 | 다음에 할 일 |
| --- | --- | --- | --- |
| 1 | 초기 맵핑 | 부분 완료 | 반복 실패 frontier 완료 전환은 확인됨. home 복귀 중 최종 진입 정지 원인 확인 |
| 2 | 맵 저장 | 완료 | 자동 저장 후 home 복귀 최종 waypoint 도달 확인 |
...
1. 맵 저장 후 home 복귀 중 최종 waypoint 근처에서 정지하는 원인을 확인한다.
2. `frontier_viewpoint_clearance_cells`, distance/obstacle penalty, home waypoint/tolerance 등 세부 튜닝을 필요 시 조정한다.
3. 순찰 실패 시 복구/중단 동작을 확인한다.
4. 특정 시간 순찰 scheduler 또는 시작 트리거 방식을 결정한다.
5. 화재 감지 후 대응 방식을 결정한다.
6. 원버튼 자동 실행 전체 시나리오를 검증한다.
